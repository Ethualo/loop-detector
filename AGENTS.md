...
- Claude Code는 커밋된 `.claude/settings.json`을 프로젝트 설정으로 읽는다. `PostToolUseFailure` payload의 `session_id`/`tool_name`/`error`만으로 세션별 임시 상태 파일에 연속 실패를 직접 추적하고, 반복 실패면 stderr + exit 2로 Claude에게 경고한다.
- Codex는 커밋된 `.codex/hooks.json`을 공식 프로젝트 훅 위치로 읽는다. 새 Codex 세션에서 `/hooks`로 훅을 검토·신뢰하면 `PostToolUse` payload의 `session_id`/`tool_name`/`tool_response`만으로 Bash non-zero를 세션별로 3회 추적해 `additionalContext` 경고를 준다. 전역 설정 수정은 필요 없다.
...
- **2단계(실시간, 0.1.2부터)**: Claude Code와 Codex 둘 다 transcript를 재파싱하지 않고, 훅 payload 자체의 필드(Claude: `session_id`/`tool_name`/`error`, Codex: `session_id`/`tool_name`/`tool_response`)만으로 세션별 임시 상태 파일에 연속 실패 카운트를 직접 추적한다(`record_consecutive_failure` 공용 헬퍼). transcript_path를 다시 읽던 구 버전(0.1.1 이하)은 파일이 실제 호출보다 지연돼 있어 3회가 아니라 5~6회 실패해야 발화하는 버그가 있었다 — [주의사항]의 0.1.2 항목 참고. 두 경로 모두 내부 예외 시 fail-open이다.
...
- **`--plugin-dir` 경로는 이 CLI 버전(v2.1.263)에서 `PostToolUseFailure`를 대화형 세션에서 아예 등록하지 않는다** — exec-form/shell-form/`description` 필드/스크립트 내용/임계치 모두 배제하고 재현됨, `/hooks`에서도 "No hooks configured for this event". 이 리포 설정 문제가 아니라 CLI 자체 한계로 결론. 실시간 경고가 필요하면 `install` 명령의 project-hook(`.claude/settings.json`) 경로를 쓸 것 — 실측 확인됨.
- **마켓플레이스(`/plugin`) 설치 경로는 등록은 되지만 과거 transcript-lag 버그가 있었음** — 0.1.1 이하에서 3회가 아니라 실제로는 5~6회 실패해야 경고가 떴다(raw transcript로 호출 순서 직접 추적해 확인: 5번째 호출 뒤 "3 times in a row" 메시지가 뜸). 0.1.2에서 payload 기반 상태 추적으로 근본 수정됨 — 자세한 조사 과정은 [CLAUDE.md](CLAUDE.md)의 관련 항목 참고.
- **0.1.2 수정 완료**: `claude_hook_warning`을 Codex 훅과 같은 방식(payload의 `session_id`/`tool_name`/`error`만으로 세션별 임시 상태 파일 추적)으로 재작성해 transcript 재파싱을 아예 없앴다. `trailing_repeat_failure`/`extract_tool_events` 기반 판정은 삭제(오프라인 `scan`용 `iter_runs`/`detect_repeated_runs`는 그대로 유지, 영향 없음). 대가: Codex와 달리 Claude Code엔 성공을 알려주는 훅이 없어(`PostToolUseFailure`는 실패만 옴) 성공이 스트릭을 못 끊는다 — 동일 오류가 성공 사이에 끼어도 계속 카운트됨(허용된 트레이드오프, ponytail 주석 남김). 플러그인/마켓플레이스 캐시가 이 수정을 받으려면 버전 bump 필요해서 `.claude-plugin/plugin.json`·`.codex-plugin/plugin.json`·`.claude-plugin/marketplace.json` 모두 0.1.2로 올림. `python test_loop_detector.py` 전체 통과 확인.
...
- Decision: fix the PostToolUseFailure transcript-tail lag by having Claude's hook track consecutive failures off payload fields directly (session_id/tool_name/error), same approach as the already-working Codex hook, instead of re-parsing transcript_path — Reason: raw transcript inspection proved the lag was exactly reproducible (threshold 3 needed 5-6 real failures to fire) and payload-based state tracking has no transcript to lag behind.
